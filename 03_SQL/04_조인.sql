...
- inner는 생략 할 수 있다.
**************************************** */
-- select hr_join.emp.emp_id  경로를 상세하게 기술해야함.(use hr_join 안했을경우,)


-- 직원의 ID(emp.emp_id), 이름(emp.emp_name), 입사년도(emp.hire_date), 소속부서이름(dept.dept_name)을 조회
select e.emp_id, e.emp_name, e.hire_date, d.dept_name   -- join 할때, 테이블 명을 앞에 써주도록
from   emp e inner join dept d on e.dept_id = d.dept_id ;   -- emp e / dept d 테이블 명의 앞글자로 약어 설정하여 참조할 것을 명시해줌.
...
-- 'San Francisco' 에 근무(dept.loc)하는 직원의 id(emp.emp_id), 이름(emp.emp_name), 
-- 입사일(emp.hire_date)를 조회
-- TO DO
select e.emp_id, e.emp_name, e.hire_date, d.loc
from  dept d join emp e on d.dept_id = e.dept_id
...
from   emp e left join dept d +
                    on e.dept_id = d.dept_id and e.dept_id = 80;

-- emp left join dept  : emp의 모든행은 join 조건에 상관없이 나온다.
--                       dept는 join 조건이 True인 행만 나온다.

select * from dept;
        
--  직원_id(emp.emp_id)가 100, 110, 120, 130, 140인 
-- 직원의 ID(emp.emp_id),이름(emp.emp_name), 업무명(job.job_title) 을 
-- 조회. 업무명이 없을 경우 '미배정' 으로 조회
...
-- 2003년~2005년 사이에 입사한 
-- 모든 직원의 id(emp.emp_id), 이름(emp.emp_name), 업무명(job.job_title), 급여(emp.salary), 입사일(emp.hire_date),
-- 상사이름(emp.emp_name), 상사의입사일(emp.hire_date), 소속부서이름(dept.dept_name), 부서위치(dept.loc)를 조회. + 급여등급(salary_grade.grade)
select  e.emp_id, e.emp_name,
        j.job_title, e.salary, e.hire_date,
        m.emp_name as "상사이름", m.hire_date as "상사 입사일",
        d.dept_name, d.loc, s.grade
from    emp e left join job j on e.job_id = j.job_id
			  left join emp m on e.mgr_id = m.emp_id
              left join dept d on e.dept_id = d.dept_id
              left join salary_grade s on e.salary 
