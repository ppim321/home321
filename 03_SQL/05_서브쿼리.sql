...
where  job_id = (select job_id from emp where emp_id = 120);
               -- 서브 쿼리는 괄호 바깥의 실행문과 별개로 단독 실행되는 비상관 쿼리이다.
...
where  (job_id, mgr_id) = (select job_id, mgr_id 
                           from emp where emp_id=150);

-- 직원들 중 급여(emp.salary)가 전체 직원의 평균 급여보다 적은 
-- 직원들의 id(emp.emp_id), 이름(emp.emp_name), 급여(emp.salary)를 조회. 
select  emp_id, emp_name, salary
from    emp
where   salary < (select avg(salary) from emp) -- 집계함수는 where 절에서 사용 불가  avg(salary);
...
from (select d.dept_name, 
   ...
   -- from (subquery) -> subquery 실행 talbe 로 부터 조회  
   -- inline view
   -- t ; 마지막에 반드시 별칭을 지정해야한다.
/**************************************************************************
**************************************************************************
***/

--  급여(emp.salary)가장 많이 받는 직원이 속한 
...
-- 업무(emp.job_id)가 'IT_PROG' 인 직원들 중 
-- 가장 많은 급여를 받는 직원보다 더 많은 급여를 받는 
...
from   emp
where  mgr_id in (select emp_id from emp where emp_name = 'Alexander');  -- 다중행의 = 이퀄 연산은 오류가 남.
...
-- 직원 ID(emp.emp_id)가 101, 102, 103 인 
-- 직원들 보다 급여(emp.salary)를 많이 받는 직원의 모든 정보를 조회.
select * from emp
where salary >all (select salary from emp         -- 서브 쿼리의 값보다 모두 커야하는 조건을 만족하기 위해   >all 
				where emp_id in (101, 102, 103));  
-- where salary > (select max salary from emp         -- max
-- 				   where emp_id in (101, 102, 103));  

-- 직원 ID(emp.emp_id)가 101, 102, 103 인 직원들 중 
-- 급여가 가장 적은 직원보다 급여를 많이 받는 직원의 모든 정보를 조회.
select * from emp
where salary >any (select salary from emp        
				    where emp_id in (101, 102, 103));  
-- where salary > (select min salary from emp         -- min         
-- 				   where emp_id in (101, 102, 103));  
...
- 메인쿼리문 테이블의 값을 where절의 subquery에서 참조한다.
	- 메인 쿼리의 where실행에서 한 행씩 조회 대상인지 검사하면서 
      subquery가 실행되는데 이때 현재 검사중인 그 행의 컬럼값을 subquery가 사용한다.
* *************************************************************************************************/
...
where  salary =  (select max(salary)
	...
	-- e1 테이블의 행을 where 에서 조회할 때 마다, Subquery를 실행.
    -- Subquery 의 e1.dept_id는 e1 의 where절에서 체크한 행의 dept_id값을 의미한다.
;
