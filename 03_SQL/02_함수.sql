...
- 문자열관련 함수
 ...
 concat(v1, v2[, ..]) - (입력 값은 문자가 아니여도) 값들을 합쳐 하나의 문자열로 반환
...
select concat(emp_name, '-', dept_name) from emp;
...
************************************************************************** */
select trim('     aaaa      ');
select round(3.14159234563); -- 직접 값을 넣어 처리해줌(테이블 없이..) -- 자릿수를 생략시, 기본값 = 0 으로 출력, 정수 처리함.00000000
select round(3.14159234563, 2); -- 소숫점 2자리 이하에서 반올림. (양수: 소숫점 이하)
select round(398.4854674, -1); -- 음수: 정수부
--    3   9   8 . 4   8   5
...
select emp_id,
       emp_name,
       salary,
       ceil(salary * 1.15) as "SAL_RAISE" -- 정수표기 출력
from   emp;
...
select emp_id,
       ...
       floor(salary - (salary * 1.15)) as "diff"   -- 계산해서 만든 컬럼과는...?
from emp;
...
select adddate(curdate(), interval 3 month); -- 오늘부터 3개월 이후 날짜?
select adddate(curdate(), interval -3 month);
select subdate(curdate(), interval 3 month);

select adddate(curdate(), interval 3 year);
select adddate(curdate(), interval 3 quarter); -- 3분기 이후(9개월 이후의 날짜)
select adddate(curdate(), interval 43 day);

-- 날짜 차이
select datediff(curdate(), '2020-10-7'); # 입력 날짜로부터 현재 날짜를 뺸 일수를 출력으로 보여줌.
...
from    emp
order by  2 desc;    -- 2번째 컬럼의 오름차순으로 정렬됨.
...
************************************************************************************* */
select null;
select ifnull(null, "값 없음");
select ifnull("홍길동", "값 없음");
...
from   emp
order by 4; -- 정렬시 select 절에서 처리괸 값 기준으로 정렬.
-- where  dept_name is null;

-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 커미션 (salary * comm_pct)을 조회.
-- 커미션이 없는 직원은 0이 조회되록 한다.
select emp_id,
       emp_name,
       format(salary, 2) as "salary",  # 3단위당, 출력.  2 : 소수점 2자리까지만 나오게함.
       comm_pct,
...
***********************************************/
-- 시간을 정수형태로 변환   
select cast(curtime() as signed); -- 시간을 하나의 숫자료 붙여서 표현. 144051 <--  14:40:51
select cast(cast(curtime() as signed) as char);
...
select salary, if(salary >= 10000, '1등급', '2등급') as "급여등급" 
from emp;



-- EMP 테이블에서 업무(job)이 'AD_PRES'거나 'FI_ACCOUNT'거나 'PU_CLERK'인 
-- 직원들의 ID(emp_id), 이름(emp_name), 업무(job)을 조회.  
...
select emp_id, emp_name, 
       ...
       case job when 'AD_PRES' then '대표'
                ...
                else job end as "job2" -- 반환값의 컬럼명을 지정시 --> 원래값이 반환됨.
from emp
...
select  dept_name,
        case dept_name when 'IT' then salary * 0.1
                       when 'Shipoping' then salary * 0.2
...
select emp_id,  -- 쉼표(,) 작성시 컬럼이 바뀜에 주의할 것.
       emp_name,
       salary,
